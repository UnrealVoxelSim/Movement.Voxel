...
`Controller` implements `Movement.Api::IIntentReceiver` and exclusively owns `InputComponent` mutation. Each update
consumes `InputComponent` only when its tick matches the current `StepContext`; stale input becomes
